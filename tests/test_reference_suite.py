...
from memory_frontier.hashing import (
    SCIENTIFIC_HASH_FLOAT_DECIMALS,
    SCIENTIFIC_HASH_SCHEME,
    finite_horizon_card_scientific_sha256,
    theory_card_scientific_sha256,
)
from memory_frontier.lab import theory_card


def test_frozen_reference_suite_recomputes_scientific_hashes():
    manifest = json.loads(Path("data/reference_suite.json").read_text())
    assert manifest["scientific_hashing"] == {
        "scheme": SCIENTIFIC_HASH_SCHEME,
        "float_decimals": SCIENTIFIC_HASH_FLOAT_DECIMALS,
    }
    ...
    for name, entry in manifest["sources"].items():
        ...
        assert (
            theory_card_scientific_sha256(card)
            == entry["theory_card_scientific_sha256"]
        ), name

        if name == "horizon_switch":
            finite = finite_horizon_card(source, 2, list(range(1, 33)))
            assert (
                finite_horizon_card_scientific_sha256(finite)
                == entry["finite_horizon_card_scientific_sha256"]
            )
